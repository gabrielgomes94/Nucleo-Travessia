...
<div id="container">
	<div class="row">
		<div class="col-md-8 posts">	
			...
			<?php if ( is_day() ) : ?>
				<h1 class="page-title">
					<?php printf( __( 'Arquivos do dia: <span>%s</span>', 'seu-template' ), get_the_time(get_option('date_format')) ) ?>
				</h1>
			<?php elseif ( is_month() ) : ?>
				<h1 class="page-title">
					<?php printf( __( 'Arquivos do mês: <span>%s</span>', 'seu-template' ), get_the_time('F Y') ) ?>
				</h1>
			<?php elseif ( is_year() ) : ?>
				<h1 class="page-title">
					<?php printf( __( 'Arquivos do ano: <span>%s</span>', 'seu-template' ), get_the_time('Y') ) ?>
				</h1>
			<?php elseif ( isset($_GET['paged']) && !empty($_GET['paged']) ) : ?>
				<h1 class="page-title">
					<?php _e( 'Arquivos do Blog', 'seu-template' ) ?>
				</h1>
			<?php endif; ?>
			<?php rewind_posts(); ?>
