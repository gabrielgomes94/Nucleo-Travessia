...
function pm_multiple_friends_request_delete(uid)
{
    jQuery('.pm-request-friends-select-checkbox').each(function (index, element) { //Validation for number type custom field
		var isChecked = jQuery(this).is(':checked');
                ...
                if(isChecked)
                {
                    pm_reject_friend_request(uid,u2,this);
                }
...
jQuery(document).on('click','#pg-myfriends ul li .page-numbers',function(event){
...
jQuery(document).on('click','#pg-requests-sent ul li .page-numbers',function(event){
    event.preventDefault();
  // console.log(jQuery(this).text());
   var link = jQuery(this).attr("href");
   if(link !== undefined)
...
jQuery(document).on('click','#pg-friend-requests ul li .page-numbers',function(event){
    event.preventDefault();
  // console.log(jQuery(this).text());
   var link = jQuery(this).attr("href");
   if(link !== undefined)
